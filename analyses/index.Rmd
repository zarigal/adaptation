---
title: "Redcedar Adaptation Data Analyses and Visualization"
...
|             |           |            |            |            |
![](https://raw.githubusercontent.com/zarigal/adaptation/refs/heads/main/images/IMG_1915.JPG)
...
```{r message=FALSE}
treesno.condition.zone <- trees.no.browse %>% filter(Elk.Browse == "N") %>% group_by(Seed.Zone,Final.Health) %>% summarize(n=n_distinct(Tree.Number)) %>% filter(Final.Health!="NA")
...
The below plot shows the number of trees that fall under each health condition sorted by seed zone and planting site for the most recent year's observations.

```{r message=FALSE}
treesno.condition <- trees.no.browse %>% filter(Measurement.Year == 2025) %>% group_by(Site, Seed.Zone,Final.Health) %>% summarize(n=n_distinct(Tree.Number)) %>% filter(Final.Health!="NA")

treesno.condition$Final.Health <- factor(treesno.condition$Final.Health, levels=c('Alive','Dead','Dying','Missing'))

ggplot(treesno.condition,aes(x = reorder(Site, -n),n,fill=Final.Health))+facet_wrap(~Seed.Zone)+geom_col(position = position_dodge2(preserve = "single",reverse=TRUE))+theme_bw()+coord_flip() + scale_y_continuous(expand = expansion(mult = c(0.05, 0.1))) +scale_fill_manual(values=c( "#009E73","#56B4E9","#E69F00","#CC79A7"))+labs(x="Site",y="Number of Trees", title = "Health Condition by Seed Zone and Site",fill="Health Condition") +geom_text(aes(label=n,group=Final.Health),position = position_dodge2(width=0.9,preserve = "single",reverse=TRUE),hjust=-0.1) + theme(plot.title = element_text(hjust = 0.5))
```
...
```{r message=FALSE}
trees.condition.zone <- trees %>% filter(Measurement.Year == 2025) %>% group_by(Seed.Zone,Final.Health) %>% summarize(n=n_distinct(Tree.Number)) %>% filter(Final.Health!="NA")
...
```{r message=FALSE}
trees.condition <- trees %>% filter(Measurement.Year == 2025) %>% group_by(Site, Seed.Zone,Final.Health) %>% summarize(n=n_distinct(Tree.Number)) %>% filter(Final.Health!="NA")
...
```{r}
ggplot(trees,aes(Tree.Height..cm.,fill=Seed.Zone))+geom_histogram()
```

```{r}
ggplot(trees,aes(Tree.Diameter..mm.,fill=Seed.Zone))+geom_histogram()
```
...
```{r}
#Check normality and equality of variances within data 
car::leveneTest(Tree.Height..cm.~as.factor(Seed.Zone), data=trees.no.browse)
car::leveneTest(Tree.Diameter..mm.~as.factor(Seed.Zone), data=trees.no.browse)
#p = .6003, Not enough evidence to suggest there is not equality of variances, assume variances are equal

#2a check normality
height.lm=lm(Tree.Height..cm.~as.factor(Seed.Zone), data = trees.no.browse)
ggqqplot(residuals(height.lm))
hist(residuals(height.lm))

diameter.lm=lm(Tree.Diameter..mm.~as.factor(Seed.Zone), data = trees.no.browse)
ggqqplot(residuals(diameter.lm))
hist(residuals(diameter.lm))
#Normailty assumed, qq plot of the residuals generally follows line of best fit and
#histogram shows mainly normal distribution
```