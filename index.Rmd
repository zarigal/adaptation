...
#merge two sheets together based on tree number

ntn2025 <- sheet2025 %>%
  mutate(`New Tree Number` = coalesce(`New Tree Number`,`Tree Number`))
         
#merged2025 <- rough2025 %>%
#full_join(rough2025, by = intersect(colnames(sheet2025), colnames(rough2025))) %>%
 # group_by(`New Tree Number`)
#merged seed zone from 2025 to merged sheet
ntn2025 <- ntn2025 %>% group_by(ntn2025$`New Tree Number`, ntn2025$Site) %>%
  merge(rough2025, ntn2025[, c("New Tree Number", "Seed Zone")], by= "New Tree Number", all.x = TRUE) %>% ungroup()

rough2025$`Seed Zone` <- rough2025$`Seed Zone`[match(ntn2025$Site, ntn2025$`New Tree Number`)]

y4 <- merged2025 %>% select(-c("Seed Zone.x"))
y4 <- y4 %>%
  rename("Seed Zone" = "Seed Zone.y")
...
y4 <- y4 %>% 
  ...
  rename("Tree.Height..cm." = "Tree Height (cm)") %>%
  rename("Tree.Diameter..mm." = "Tree Diameter (mm)") %>%
  rename("Seed.Zone" = "Seed Zone") %>%
  rename("Date.Measured" = "Date Measured")
...
y1$Date.Measured <- as.Date(y1$Date.Measured,"%m/%d/%y")
y1$Date.Planted <- as.Date(y1$Date.Planted,"%m/%d/%y")
y2$Date.Measured <- as.Date(y2$Date.Measured,"%m/%d/%y")
y3$Date.Measured <- as.Date(y3$Date.Measured,"%m/%d/%y")
...
trees.bind <- trees.bind %>%  group_by(Tree.Number, Site, Seed.Zone) %>% mutate(Final.Health = if_else(Measurement.Year == 2025, ifelse(is.na(Health),first(na.omit(Health)),Health),first(na.omit(Health)),Health)) %>% ungroup()
```
...
P1 <- ggplot(trees.wide.pos.all,aes(Seed.Zone,y1.height.growth,fill=Seed.Zone)) +geom_violin() +geom_boxplot() +theme_bw()+guides(fill=FALSE) +scale_fill_manual(values=c("#E69F00", "#56B4E9")) +labs(y= "Year 1 Height Growth (cm)", x= "Seed Source",title="Year 1 Growth") + theme(plot.title = element_text(hjust = 0.5))
P2 <- ggplot(trees.wide.pos.all,aes(Seed.Zone,y2.height.growth,fill=Seed.Zone)) +geom_violin() +geom_boxplot() +theme_bw()+guides(fill=FALSE) +scale_fill_manual(values=c("#E69F00", "#56B4E9")) +labs(y= "Year 2 Height Growth (cm)",x= "Seed Source",title="Year 2 Growth") + theme(plot.title = element_text(hjust = 0.5))
P3 <- ggplot(trees.wide.pos.all,aes(Seed.Zone,total.height.growth,fill=Seed.Zone)) +geom_violin() +geom_boxplot() +theme_bw()+guides(fill=FALSE) +scale_fill_manual(values=c("#E69F00", "#56B4E9")) +labs(y= "Total Height Growth (cm)",x= "Seed Source",title="Cumulative Growth") + theme(plot.title = element_text(hjust = 0.5))
...
```{r warning=FALSE}
P1 <- ggplot(trees.wide.pos.all,aes(Seed.Zone,y1.diameter.growth,fill=Seed.Zone)) +geom_violin() +geom_boxplot() +theme_bw()+guides(fill=FALSE) +scale_fill_manual(values=c("#E69F00", "#56B4E9")) +labs(y= "Year 1 Diameter Growth (mm)", x= "Seed Source" ,title="Year 1 Growth") + theme(plot.title = element_text(hjust = 0.5)) 
P2 <- ggplot(trees.wide.pos.all,aes(Seed.Zone,y2.diameter.growth,fill=Seed.Zone)) +geom_violin() +geom_boxplot() +theme_bw()+guides(fill=FALSE) +scale_fill_manual(values=c("#E69F00", "#56B4E9")) +labs(y= "Year 2 Diameter Growth (mm)",x= "Seed Source",title="Year 2 Growth") + theme(plot.title = element_text(hjust = 0.5))
P3 <- ggplot(trees.wide.pos.all,aes(Seed.Zone,total.diameter.growth,fill=Seed.Zone)) +geom_violin() +geom_boxplot() +theme_bw()+guides(fill=FALSE) +scale_fill_manual(values=c("#E69F00", "#56B4E9")) +labs(y= "Total Diameter Growth (mm)",x= "Seed Source",title="Cumulative Growth") + theme(plot.title = element_text(hjust = 0.5))
