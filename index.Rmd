...
full_join(rough2025, by = intersect(colnames(sheet2025), colnames(rough2025)), na_matches = c("never")) %>%
  group_by(`New Tree Number`)
#need to figure out how to keep seed zone from 2nd sheet

y1 <- y1 %>% mutate(Temp.Tree.Number = row_number())
y4$`Tree Number`<- y4$Tree.Number 

y1 <- y1 %>% select(-c("Year.Measured","Color"))
y2 <- y2 %>% select(-c("Year.Measured","Color"))
y3 <- y3 %>% select(-c("Year.Measured"))
y4 <- y4 %>% select(-c("Year Measured","Color"))
