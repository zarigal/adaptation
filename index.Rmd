...
library(readxl)
y4 <- read_excel("data/2025 Measurements.xlsx")

y1$Measurement.Year <- 2022
y2$Measurement.Year <- 2023
y3$Measurement.Year <- 2024
y4$Measurement.Year <- 2025

y1 <- y1 %>% mutate(Temp.Tree.Number = row_number())

y1 <- y1 %>% select(-c("Year.Measured","Color","Temp.Tree.Number"))
y2 <- y2 %>% select(-c("Year.Measured","Color"))
y3 <- y3 %>% select(-c("Year.Measured","New.Tree.Number"))
