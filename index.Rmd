...
library(readxl)
y2 <- read_excel("data/2023 MeasurementsCorrected.xlsx")
y3 <- read_excel("data/2024 Measurementsv3.xlsx")
...
y1 <- y1 %>% 
  ...
  rename("Date Planted" = "Date.Planted")
```
...
```{r warning=FALSE}
ggplot(trees.wide.pos.all,aes(`Seed Zone`,total.diameter.growth,fill=Site)) +geom_boxplot() +theme_bw() +scale_fill_manual(values=c("#E69F00", "#56B4E9", "#009E73", "#9e0000")) + labs(y= "Total Diameter Growth (mm)",x= "Seed Source",title="Cumulative Diameter Growth by Site") + theme(plot.title = element_text(hjust = 0.5))
```
...
library(car)
car::leveneTest(`Tree Height (cm)`~as.factor(`Seed Zone`), data=trees)
car::leveneTest(`Tree Diameter (mm)`~as.factor(`Seed Zone`), data=trees)
#p = .6003, Not enough evidence to suggest there is not equality of variances, assume variances are equal

#2a check normality
math.lm=lm(MathScore~as.factor(FivePlusHousehold)+as.factor(PublicPrivateSchool), data = ECLSK2011_Clean)
install.packages("ggpubr")
library(ggpubr)
ggqqplot(residuals(math.lm))
hist(residuals(math.lm))
