---
title: "Redcedar Adaptation Data Wrangling"
...
|             |           |            |            |            |
![](https://raw.githubusercontent.com/zarigal/adaptation/refs/heads/main/images/IMG_1906.JPG)
...
* The Data
  ...
  + 2026 Measurements (4 years after planting)
...
y5 <- read_excel("WRC Tree Measurements 2025.xlsx", sheet = "2026")
```
...
y5 <- cbind(y5, Measurement.Year = 2026)
```
...
y5$`Date Measured` <- as.Date(y5$`Date Measured`,format = "%m/%d/%y")
```

Merge all years' data frames together.
```{r}
y5$`Tree Diameter (mm)` <- as.numeric(y5$`Tree Diameter (mm)`) 
y5$`Tree Height (cm)` <- as.numeric(y5$`Tree Height (cm)`) 
trees <- bind_rows(y1,y2) %>% bind_rows(.,y3) %>% bind_rows(.,y4) %>% bind_rows(.,y5) %>% filter(Site!="" & Site!="Puyallup") %>% droplevels()
```
...
1. If previous year's health condition is "Dead" then change following year's health condition to dead
